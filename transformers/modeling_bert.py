...
class BertSelfAttention(nn.Module):
    ...
    def set_stage(self,stage):
        self.query.set_stage(stage)
        self.value.set_stage(stage)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.adapter=Adapter("A1",config.hidden_size,128,config.hidden_size,AdapterConfig)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.adapter_LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, hidden_states, input_tensor):
        dense_hidden_states = self.dense(hidden_states)
        adapter_hidden_states = self.adapter(input_tensor,dense_hidden_states)
        hidden_states = self.dropout(dense_hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        adapter_hidden_states = self.adapter_LayerNorm(adapter_hidden_states + input_tensor)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        layer_output,adapter_layer_output = self.output(intermediate_output, attention_output)
        outputs = (layer_output,) + outputs
        adapter_outputs = (adapter_layer_output,) + outputs
        return outputs,adapter_outputs
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs,adapter_layer_outputs = layer_module(
    ...
    def adaptive_forward(self, hidden_states, current_layer, attention_mask=None, head_mask=None):
        layer_outputs,adapter_layer_outputs = self.layer[current_layer](hidden_states, attention_mask, head_mask[current_layer])

        hidden_states = layer_outputs[0]
        adapter_hidden_states = adapter_layer_outputs[0]
...
class BertModel(BertPreTrainedModel):
    ...
    Examples::
        ...
        if self.training:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                if i==self.config.num_hidden_layers-1:
                    ...
                    pooled_output = self.pooler(adapter_encoder_outputs)
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                if i == self.config.num_hidden_layers - 1:
                    ...
                    pooled_output = self.pooler(adapter_encoder_outputs)
                feat = output_layers[i](pooled_output)
                cur_M = output_layers[i].ori_M
                logits = torch.matmul(feat, cur_M)
                if i == out_id:
                    break
        ...
        #         if i == 3:
        #             logits_4=np.argmax(logits.detach().cpu().numpy(), axis=1)
        #         if i == 4:
        #             logits_5=np.argmax(logits.detach().cpu().numpy(), axis=1)
        #             if logits_5==logits_4:
        #                 output_id=5
        #                 break
        #     res = [logits]

        # elif self.patience==100:
        #     preds=[]
        #     import numpy as np
        #     for i in range(self.config.num_hidden_layers):
        #         encoder_outputs = self.encoder.adaptive_forward(encoder_outputs,
        ...
        #         pooled_output = self.pooler(encoder_outputs)
        #         logits = output_layers[i](pooled_output)
        #         #labels = logits.detach().argmax(dim=1).cpu().numpy()
        #
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifiers = nn.ModuleList(
            [ETF_Classifier(feat_in=768, num_classes=config.num_labels) for _ in range(config.num_hidden_layers)])
    ...
    Examples::
        ...
        return logits,outputs,output_id
